/**
 * ════════════════════════════════════════════════════════════════
 * SAGI Finance — PLUS MODÜLÜ
 * ────────────────────────────────────────────────────────────────
 * Bu dosya SAGI Plus premium özellikleri içerir.
 * index.html'e HENÜZ eklenmemiştir — ödeme sistemi hazır olduğunda
 * entegre edilecektir.
 *
 * İÇERİK:
 *   1. SAGIAssistant  — Finansal yorum motoru (sagi-assistant.js'den taşındı)
 *   2. SavingsScore   — Tasarruf skoru (index.html Analytics'ten taşındı)
 *   3. Forecast       — Gelecek ay tahmini (index.html Analytics'ten taşındı)
 *   4. App.Plus       — Plus durum yönetimi ve feature flag sistemi
 * ════════════════════════════════════════════════════════════════
 */
...
/* ══════════════════════════════════════════════════════════════
   BÖLÜM 1 — SAGI ASSISTANT (Finansal Yorum Motoru)
   Orijinal: sagi-assistant.js
   ══════════════════════════════════════════════════════════════ */
...
window.SAGIAssistant = (function () {
  ...
  function _buildTips(ctx) {
    ...
    if (savingsRate < 20) pool.push({ text: L.tip5030 });
    if (topCats.length && totalExp > 0 && (topCats[0][1] / totalExp) > 0.35) {
      pool.push({ text: L.tipTopCat(topCats[0][0], fmt, topCats[0][1] * 0.1) });
    }
    if (recurring && recurring.length >= 4) pool.push({ text: L.tipRecurring(recurring.length) });
    if (!budgets || budgets.length === 0) pool.push({ text: L.tipBudget });
    if (months.length >= 3) {
      const last3 = months.slice(-3);
      if (last3[2].exp < last3[0].exp) pool.push({ text: L.tipMomentum });
    }
    if (totalInc > 0 && totalInc < 10000 && savingsRate < 15) pool.push({ text: L.tipIncome });
    if (savingsRate < 10) pool.push({ text: L.tipEmergency });
  ...
  function render(ctx) {
    ...
    if (insightEl) {
      ...
      if (tagsEl) tagsEl.innerHTML = tags.map(tag => `<span class="ai-tag ${tag.cls}">${tag.label}</span>`).join('');
    }
    const tipsEl = document.getElementById('aiInsightTips');
    if (tipsEl) tipsEl.innerHTML = _buildTips(ctx);
    const monthlyEl = document.getElementById('sagiMonthlyCards');
    if (monthlyEl) monthlyEl.innerHTML = _buildMonthlyCards(ctx);
    const breakdownEl = document.getElementById('sagiSavingsBreakdown');
    if (breakdownEl) breakdownEl.innerHTML = _buildSavingsBreakdown(ctx);
  }
...
/* ══════════════════════════════════════════════════════════════
   BÖLÜM 2 — SAVINGS SCORE
   Orijinal: index.html > Analytics._renderSavingsScore()
   ══════════════════════════════════════════════════════════════ */

window.SAGISavingsScore = {
  render(savingsRate) {
    const wrap    = document.getElementById('savingsScoreSvg');
    const gradeEl = document.getElementById('savingsScoreGrade');
    const descEl  = document.getElementById('savingsScoreDesc');
    if (!wrap) return;

    const t = (key) => window.App && App.I18n ? App.I18n.t(key) : key;

    const displayPct = Math.max(0, Math.min(100, savingsRate));
    const score = displayPct;
    const r = 42, cx = 55, cy = 55;
    const circumference = 2 * Math.PI * r;
    const dash = (score / 100) * circumference;
    const gap  = circumference - dash;
    const colorHex = score >= 70 ? '#22a06b' : score >= 40 ? '#4f46e5' : score >= 20 ? '#d97706' : '#e5534b';
    const grade = score >= 80 ? t('scoreGradeExcellent') : score >= 60 ? t('scoreGradeGood') : score >= 40 ? t('scoreGradeMid') : score >= 20 ? t('scoreGradeWarn') : t('scoreGradeCrit');
    const desc  = score >= 70 ? t('scoreDescExcellent') : score >= 40 ? t('scoreDescGood') : score >= 20 ? t('scoreDescMid') : t('scoreDescCrit');

    wrap.innerHTML = `<svg viewBox="0 0 110 110">
      <circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="#e5e7eb" stroke-width="10"/>
      <circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${colorHex}" stroke-width="10"
        stroke-dasharray="${dash} ${gap}" stroke-dashoffset="0"
        stroke-linecap="round" transform="rotate(-90 ${cx} ${cy})" />
      <text x="${cx}" y="${cy - 4}" text-anchor="middle" font-size="23" font-weight="800" font-family="Outfit,sans-serif" fill="var(--text-main)">${displayPct.toFixed(0)}%</text>
      <text x="${cx}" y="${cy + 12}" text-anchor="middle" font-size="8.5" font-weight="700" fill="#9ca3af">${t('scoreSavingsLabel2')}</text>
    </svg>`;

    if (gradeEl) {
      gradeEl.textContent = grade;
      gradeEl.style.cssText = `background:${colorHex}22;color:${colorHex};border:1px solid ${colorHex}44;`;
    }
...
/* ══════════════════════════════════════════════════════════════
   BÖLÜM 3 — FORECAST (Gelecek Ay Tahmini)
   Orijinal: index.html > Analytics._renderForecast()
   ══════════════════════════════════════════════════════════════ */

window.SAGIForecast = {
  render(months) {
    const el = document.getElementById('forecastRows');
    if (!el) return;

    const t   = (key) => window.App && App.I18n ? App.I18n.t(key) : key;
    const mc  = Core.state.settings.currency || 'TRY';
    const fmt = v => Core.Utils.formatMoney(v, mc);

    if (months.length < 2) {
      el.innerHTML = `<div style="color:var(--text-muted);font-size:14px;padding:12px 0">${t('forecastMinData')}</div>`;
      return;
    }

    const weights  = months.map((_, i) => i + 1);
    const wSum     = weights.reduce((s, w) => s + w, 0);
    const wAvgInc  = months.reduce((s, m, i) => s + m.inc * weights[i], 0) / wSum;
    const wAvgExp  = months.reduce((s, m, i) => s + m.exp * weights[i], 0) / wSum;
    const last     = months[months.length - 1];
    const prev     = months[months.length - 2];
    const incTrend = last.inc - prev.inc;
    const expTrend = last.exp - prev.exp;
    const forecastInc = Math.max(0, wAvgInc * 0.7 + (last.inc + incTrend) * 0.3);
    const forecastExp = Math.max(0, wAvgExp * 0.7 + (last.exp + expTrend) * 0.3);
    const forecastNet = forecastInc - forecastExp;

    const recBase = (Core.state.recurring || []).filter(r => r.active !== false).reduce((s, r) => {
      const cur = r.currency || mc;
      return s + App.ExchangeRate.convert(r.amount || 0, cur, mc);
    }, 0);

    const incVars  = months.map(m => (m.inc - wAvgInc) ** 2);
    const expVars  = months.map(m => (m.exp - wAvgExp) ** 2);
    const expStd   = Math.sqrt(expVars.reduce((s, v) => s + v, 0) / expVars.length);
    const confPct  = Math.max(0, Math.min(95, 95 - (expStd / forecastExp * 100)));

    const now       = new Date();
    const nextMonth = new Date(now.getFullYear(), now.getMonth() + 1, 1)
      .toLocaleDateString('tr-TR', { month: 'long', year: 'numeric' });

    const row = (label, val, conf, color, note = '') => `
      <div class="forecast-row">
        <div class="forecast-label">${label}</div>
        <div>
          <div class="forecast-val" style="color:${color}">${val}</div>
          ${conf  ? `<div class="forecast-conf">%${conf.toFixed(0)} ${t('forecastConfidence')}</div>` : ''}
          ${note  ? `<div class="forecast-conf">${note}</div>` : ''}
        </div>
      </div>`;

    el.innerHTML = `
      <div style="font-size:13px;font-weight:600;color:var(--brand-accent);margin-bottom:12px;padding:8px 12px;background:rgba(79,70,229,.07);border-radius:var(--radius-sm)">
        ${nextMonth} ${t('forecastLabel')}
      </div>
      ${row(t('forecastExpIncome'),  fmt(forecastInc), confPct, 'var(--status-success)')}
      ${row(t('forecastExpExpense'), fmt(forecastExp), confPct, 'var(--status-danger)')}
      ${recBase > 0 ? row(t('forecastFixedCosts'), fmt(recBase), null, 'var(--status-warn)', t('forecastFixedNote')) : ''}
      ${row(t('forecastNet'), fmt(forecastNet), null, forecastNet >= 0 ? 'var(--status-success)' : 'var(--status-danger)', forecastNet < 0 ? t('forecastNetWarn') : '')}`;
  }
};


/* ══════════════════════════════════════════════════════════════
   BÖLÜM 4 — APP.PLUS (Plus Durum Yönetimi)
   ══════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  const Plus = {
    ...
    isPlusUser() {
      const key = (Core.state && Core.state.settings && Core.state.settings.syncKey) || '';
      return key.startsWith(PLUS_PREFIX);
    },

    getPlan() {
      if (!this.isPlusUser()) return null;
      return (Core.state.settings && Core.state.settings.plusPlan) || 'lifetime';
    },
    ...
    hasFeature(feature) {
      if (!this.isPlusUser()) return false;
      return this._features.includes(feature);
    },

    // ── Dev / Test ──────────────────────────────────────────────
    async devActivate() {
      if (!window._SAGI_DEV) { console.warn('[Plus] Dev modu aktif değil. window._SAGI_DEV = true yapın.'); return; }
      if (!Core.state.settings.syncKey) { console.warn('[Plus DEV] Önce bir syncKey oluştur.'); return; }
      const oldKey = Core.state.settings.syncKey;
      if (oldKey.startsWith(PLUS_PREFIX)) { console.log('[Plus DEV] Zaten Plus:', oldKey); return; }
      const newKey = PLUS_PREFIX + oldKey;
      Core.Cloud.detachListener();
      ...
      await Core.Cloud.forcePush();
      try {
        await window._fbDB.collection('users').doc(oldKey.replace(/-/g, '')).delete();
        console.log('[Plus DEV] Eski doküman silindi:', oldKey);
      } catch(e) { console.warn('[Plus DEV] Eski doküman silinemedi:', e); }
      Core.Cloud.attachListener();
      console.log('[Plus DEV] ✅ Plus aktif. Yeni key:', newKey);
    },

    async devDeactivate() {
      if (!window._SAGI_DEV) { console.warn('[Plus] Dev modu aktif değil.'); return; }
      const key = (Core.state.settings && Core.state.settings.syncKey) || '';
      if (!key.startsWith(PLUS_PREFIX)) { console.log('[Plus DEV] Zaten Plus değil.'); return; }
      const newKey = key.replace(PLUS_PREFIX, '');
      Core.Cloud.detachListener();
      Core.state.settings.syncKey = newKey;
      Core.state.settings.lastModified = Date.now();
      localStorage.setItem(Core.DB.key, JSON.stringify(Core.state));
      await Core.Cloud.forcePush();
      try {
        await window._fbDB.collection('users').doc(key.replace(/-/g, '')).delete();
        console.log('[Plus DEV] PLUS dokümanı silindi.');
      } catch(e) { console.warn('[Plus DEV] PLUS dokümanı silinemedi:', e); }
      Core.Cloud.attachListener();
      console.log('[Plus DEV] Plus devre dışı. Yeni key:', newKey);
    },
  };
  ...
  console.log('[SAGI] Plus modülü yüklendi.');
})();